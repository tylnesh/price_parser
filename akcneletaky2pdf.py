...
def main() -> int:
    ...
    ap.add_argument("url", help="Flyer URL, e.g. https://lidl.akcneletaky.sk/lidl-letak-od-2-1_25428/")
    ap.add_argument("-o", "--out", default=None, help="Output PDF path (default: derived from URL)")
    ap.add_argument("-d", "--dir", default=None, help="Directory for downloaded images (default: ./<outstem>_images)")
    ...
    out_pdf = Path(args.out) if args.out else Path(default_out_name(args.url))
    out_dir = Path(args.dir) if args.dir else Path(f"{out_pdf.stem}_images")

    s = requests.Session()
    s.headers.update({"User-Agent": args.ua})

    try:
        pages = crawl_all_pages(s, args.url, args.timeout, args.quiet)
    except Exception as e:
        print(f"ERROR: failed while crawling flyer pages: {e}", file=sys.stderr)
        return 2

    if not pages:
        print(
            "ERROR: no pages found. Site structure may have changed.\n"
    ...
    try:
        ...
        print(f"ERROR: failed to download images: {e}", file=sys.stderr)
        return 4
